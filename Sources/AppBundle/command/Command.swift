...
extension Command {
    @MainActor
    @discardableResult
    func run(_ env: CmdEnv, _ stdin: CmdStdin) async throws(CancellationError) -> CmdResult {
...
extension [Command] {
    @MainActor
    func runCmdSeq(_ env: CmdEnv, _ io: sending CmdIo) async throws(CancellationError) -> Bool {
        var isSucc = true
        for command in self {
    ...
    @MainActor
    func runCmdSeq(_ env: CmdEnv, _ stdin: CmdStdin) async throws(CancellationError) -> CmdResult {
